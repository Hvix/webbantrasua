...
if ($select_orders->rowCount() > 0) {
    ...
    $pdf->Cell(0, 10, 'Trà Sữa HVT', 0, 1, 'C');
    $pdf->SetFont('dejavusans', '', 12);
    $pdf->Cell(0, 10, 'Address: TP Hồ Chí Minh', 0, 1, 'C');
    $pdf->Cell(0, 10, 'Phone: 0933663855', 0, 1, 'C');
    $pdf->Cell(0, 10, 'Email: hvt@gmail.com', 0, 1, 'C');
    $pdf->Ln(10);
    ...
    if (!empty($totalProducts)) {
        ...
        $pdf->Cell(0, 10, 'Name: ' . $fetch_orders['name'], 0, 1, 'L'); // Loại bỏ convertToNonUnicode
        $pdf->Cell(0, 10, 'Email: ' . $fetch_orders['email'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Address: ' . $fetch_orders['address'], 0, 1, 'L');
        $pdf->Ln(10);

        // Add product details in a table
        $pdf->Cell(20, 10, 'Stt', 1);
        ...
        foreach ($productsArray as $productInfo) {
            ...
            if (count($productParts) == 2) {
                ...
                $quantity = trim(explode(" x ", $productParts[1])[1]);
                $price = trim(explode(" x ", $productParts[1])[0]);

                // Loại bỏ dấu ")"
                $quantity = rtrim($quantity, ')');

                $select_product_id = $conn->prepare("SELECT id, price FROM `products` WHERE name = ?");
                $select_product_id->execute([$productName]); // Loại bỏ convertToNonUnicode
                $product = $select_product_id->fetch(PDO::FETCH_ASSOC);

                if ($product) {
                    $productID = $product['id'];

                    // Tính giá trị cho cột Total price
                    $asdValue = $price * $quantity;

                    // Add row to the table
                    $pdf->Cell(20, 10, $counter, 1);
                    $pdf->Cell(30, 10, $productID, 1);
                    $pdf->Cell(60, 10, $productName, 1); // Loại bỏ convertToNonUnicode
                    $pdf->Cell(30, 10, $price, 1);
                    $pdf->Cell(20, 10, $quantity, 1);
                    $pdf->Cell(30, 10, $asdValue, 1);
                    $pdf->Ln();
